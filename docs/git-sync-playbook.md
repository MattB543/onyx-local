...
| Ollama LiteLLM registration fix | `backend/onyx/llm/litellm_singleton/config.py` (skip Ollama runtime model lookup) | No test — commit 167df04c38; keep when upstream reshapes LiteLLM init |
| Pasted-file double upload fix | `web/src/views/AppPage.tsx`, `web/src/app/nrf/NRFPage.tsx` (commit 622a5eb889) | Upstream fixed the same bug in #14700 (`noPaste` on the same Dropzones, e030d3e91a) — TAKE THEIRS; row can be deleted after the 2026-09-23 sync |
