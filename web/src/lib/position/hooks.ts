...
type AppPositionType =
  ...
  | {
      location:
        ...
        | "user-settings"
        | "shared-chat"
        // Fork-only: the CRM module lives at its own pathname under /app.
        | "crm";
    };
...
class AppPosition {
  ...
  /** Fork-only: true while the user is anywhere inside the CRM module. */
  isCrm(): boolean {
    return this.value.location === "crm";
  }
...
export function useAppPosition(): AppPosition {
  ...
  return useMemo(() => {
    ...
    if (pathname.startsWith("/app/crm")) {
      return new AppPosition({ location: "crm" }, router);
    }
    if (chatId)
