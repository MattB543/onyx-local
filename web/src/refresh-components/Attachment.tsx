...
export default function Attachments({ fileName, open }: AttachmentsProps) {
  ...
  const nameClassName = "flex flex-col px-2";
  const nameBody = (
    <>
  ...
  return (
    <div className="flex items-center border bg-background-tint-00 rounded-12 p-1 gap-1">
      ...
      {open ? (
        // The name opens the file like the expand button. It holds two lines
        // of text, so it is a div with button semantics.
        <div
          role="button"
          tabIndex={0}
          onClick={open}
          onKeyDown={clickOnKeyDown(open)}
          className={cn(nameClassName, "cursor-pointer")}
        >
          {nameBody}
        </div>
      ) : (
        <div className={nameClassName}>{nameBody}</div>
      )}
