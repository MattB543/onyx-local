    ...
    def _completion(
        ...
        try:
            ...
            def _run_attempts(max_tokens_arg: int | None) -> Any:
                # The reasoning-none learning below rebinds the effective
                # intent for the rest of the call, like upstream's inline loop.
                ...
                for i, opts in enumerate(attempts):
                    ...
                    try:
                        ...
                        if (
                            ...
                            # A name the version gate cannot place learns "none"
                            # from the 400 itself, one round trip late. Rebuilt
                            # attempts all carry it, so the loop picks up the
                            # tail and this fires once.
                            reasoning_effort = ReasoningEffort.OFF
                            forced = {
                                k: v
