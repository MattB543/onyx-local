...
class OAuthAccount(SQLAlchemyBaseOAuthAccountTableUUID, Base):
    ...
    access_token: Mapped[str] = mapped_column(EncryptedStringUnmasked(), nullable=False)  # type: ignore
...
class CrmSettings(Base):
    ...
    tier2_enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    __table_args__ = (CheckConstraint("id = 1", name="ck_crm_settings_singleton"),)
...
class CrmContact(Base):
    ...
    location: Mapped[str | None] = mapped_column(String, nullable=True)
    profile_picture_file_id: Mapped[str | None] = mapped_column(String, nullable=True)
    created_by: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("user.id", ondelete="SET NULL"),
        nullable=True,
...
class CrmContactOwner(Base):
    ...
    __table_args__ = (Index("ix_crm_contact_owner_user_id", "user_id"),)
...
class CrmTag(Base):
    ...
    __table_args__ = (Index("ix_crm_tag_name_lower", func.lower(name), unique=True),)
